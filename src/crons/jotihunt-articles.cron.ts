...
export default async function retrieveJotihuntArticles() {
    ...
    await Article.deleteMany({}).catch((error) => { logger.error("Error deleting articles from database:", error) });
    await Article.insertMany(articles).catch((error) => { logger.error("Error inserting articles into database:", error) });
